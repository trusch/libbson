...
BSON::Value BSON::Value::fromBSON(const std::string & bson){
	...
	while(currentByte < docLen+3){
		...
		std::string name{(const char*)(docPtr+currentByte),nameEndPos-currentByte};
		if(first && name == "0"){
			docType = BSON::ARRAY;
			result.setType(docType);	
		}
		...
		switch(type){
			case BSON::UNDEFINED:
				if(docType == BSON::OBJECT) result[name] = current;
				else result.push_back(current);
				break;
			case BSON::INT32:
				if(docType == BSON::OBJECT) result[name] = *((BSON::int32*)(docPtr+currentByte));
				else result.push_back(*((BSON::int32*)(docPtr+currentByte)));
				currentByte += 4;
				break;
			case BSON::INT64:
				if(docType == BSON::OBJECT) result[name] = *((BSON::int64*)(docPtr+currentByte));
				else result.push_back(*((BSON::int64*)(docPtr+currentByte)));
				currentByte += 8;
				break;
			case BSON::DOUBLE:
				if(docType == BSON::OBJECT) result[name] = *((double*)(docPtr+currentByte));
				else result.push_back(*((double*)(docPtr+currentByte)));
				currentByte += 8;
				break;
			case BSON::BOOL:
				if(docType == BSON::OBJECT) result[name] = docPtr[currentByte]=='\x01';
				else result.push_back(docPtr[currentByte]=='\x01');
				currentByte += 1;
				break;
			case BSON::STRING:
